...
test("works", async () => {
  ...
  expect(eq(ctx, marshalPrimitive(ctx, BigInt(1)) ?? ctx.undefined, ctx.newBigInt(BigInt(1)))).toBe(
    true,
  );
  ...
  expect(
    arrayBufferEq(
      ctx,
      marshalPrimitive(ctx, arrayBuffer) ?? ctx.undefined,
      ctx.newArrayBuffer(arrayBuffer),
    ),
  ).toBe(true);
